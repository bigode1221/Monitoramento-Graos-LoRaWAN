...
static LoRaMacStatus_t VerifyTxFrame( void )
{
    ...
    if( Nvm.MacGroup2.NetworkActivation != ACTIVATION_TYPE_NONE )
    {
        ...
        MW_LOG( TS_OFF, VLEVEL_M, "[DBG-MW] VerifyTxFrame: AppDataSize=%u | DR=%d | macCmdsSize=%u\r\n",
                (unsigned int)MacCtx.AppDataSize, Nvm.MacGroup1.ChannelsDatarate, (unsigned int)macCmdsSize );

        if( ValidatePayloadLength( MacCtx.AppDataSize, Nvm.MacGroup1.ChannelsDatarate, macCmdsSize ) == false )
        {
            MW_LOG( TS_OFF, VLEVEL_M, "[DBG-MW] VerifyTxFrame: FALHOU ValidatePayloadLength!\r\n" );
